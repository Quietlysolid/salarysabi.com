-- Add opt-in tax-rule notifications and their privacy-safe funnel events.
alter table public.early_access_signups drop constraint if exists early_access_signups_source_check;
alter table public.early_access_signups add constraint early_access_signups_source_check
check (source in ('homepage','payslip_checker','employer_payroll','contributor_program','tax_updates'));

drop policy if exists "allow anonymous early-access signup" on public.early_access_signups;
create policy "allow anonymous early-access signup" on public.early_access_signups for insert to anon
with check (source in ('payslip_checker','employer_payroll','contributor_program','tax_updates') and length(email) between 5 and 254 and email=lower(email));
