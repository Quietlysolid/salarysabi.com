-- Validate contributor demand before any paid campaign is activated.
...
check (source in ('homepage','payslip_checker','employer_payroll','contributor_program'));
...
with check (source in ('payslip_checker','employer_payroll','contributor_program') and length(email) between 5 and 254 and email=lower(email));
