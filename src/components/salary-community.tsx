...
export function SalaryCommunity({ initialRanges }: { initialRanges: SalaryRange[] | null }) {
  ...
  return <div className="salary-community">
    ...
    <section className="community-ranges" aria-labelledby="community-ranges-title">
      ...
      {status === "ready" && !ranges.length && <div className="community-empty" role="status"><UsersRound aria-hidden="true" /><h3>We&apos;re building the salary picture.</h3><p>Ranges appear once at least five similar reports have been reviewed. Your salary can help build the picture.</p></div>}
