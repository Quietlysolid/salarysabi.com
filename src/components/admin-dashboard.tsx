...
export function AdminDashboard() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    const [submissionResult, jobResult, reportResult, metricResult] = await Promise.all([
    ...
    if (submissionResult.error || jobResult.error || reportResult.error || metricResult.error) { setMessage("Part of the administration dashboard could not be loaded."); return; }
  ...
  return <section className="admin-shell">
    ...
    <section className="admin-section"><h2>Pending submissions</h2>{!submissions.length && <p>No pending submissions.</p>}<div className="moderation-list">{submissions.map((item) => <article className="moderation-card" key={item.id}>
      ...
      <dl><div><dt>Salary</dt><dd>₦{Number(item.salary_min).toLocaleString()}–₦{Number(item.salary_max).toLocaleString()} {item.salary_type} / {item.salary_period}</dd></div><div><dt>Contact</dt><dd>{item.contact_email}</dd></div><div><dt>Deadline</dt><dd>{item.expires_at}</dd></div></dl>
