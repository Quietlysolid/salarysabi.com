...
const copy: Record<Mode, { title: string; intro: string }> = {
  freelancer: { title: "Freelancer tax planner", intro: "Estimate personal income tax on annual business profit after supportable business expenses." },
...
export function TaxPlanner({ mode }: { mode: Mode }) {
  ...
  return <section className="tax-planner" aria-labelledby="planner-title">
    <header><span className="eyebrow">2026 planning estimate</span><h1 id="planner-title">{copy[mode].title}</h1><p>{copy[mode].intro}</p></header>
    <div className="tax-planner-grid">
      <form className="tax-planner-form" onSubmit={(event) => event.preventDefault()}>
        <label>{mode === "foreign" ? "Annual amount in foreign currency" : mode === "investment" ? "Annual investment income" : "Annual gross revenue"}<span className="small-currency-input"><b>{mode === "foreign" ? "$" : "₦"}</b><input inputMode="numeric" value={values.revenue} onChange={(event) => set("revenue", event.target.value)} placeholder={mode === "foreign" ? "10,000" : "12,000,000"} /></span></label>
        ...
        {mode !== "investment" && <label>Annual supportable expenses<span className="small-currency-input"><b>₦</b><input inputMode="numeric" value={values.expenses} onChange={(event) => set("expenses", event.target.value)} placeholder="2,000,000" /></span><small>Do not enter personal spending. Keep invoices and records.</small></label>}
        {(mode === "freelancer" || mode === "creator") && <label>Other eligible personal deductions<span className="small-currency-input"><b>₦</b><input inputMode="numeric" value={values.deductions} onChange={(event) => set("deductions", event.target.value)} placeholder="0" /></span></label>}
        {mode === "company" && <label>Qualifying fixed assets<span className="small-currency-input"><b>₦</b><input inputMode="numeric" value={values.assets} onChange={(event) => set("assets", event.target.value)} placeholder="100,000,000" /></span></label>}
