...
export function PayrollWorkspace() {
  ...
  if (!session) return <section className="payroll-shell payroll-access payroll-entry">
    <div className="payroll-entry-intro"><span className="eyebrow">Small-team payroll</span><h1>Run payroll with clear PAYE figures.</h1><p>For Nigerian teams with up to 20 employees. Prepare payroll, review every deduction and issue payslips from one private workspace.</p>
      <ol className="payroll-feature-preview"><li><span>01</span><div><strong>Add your team</strong><small>Enter employees individually or import a CSV.</small></div></li><li><span>02</span><div><strong>Review before saving</strong><small>See gross pay, PAYE, deductions and net pay together.</small></div></li><li><span>03</span><div><strong>Keep a clear record</strong><small>Export a register and generate employee payslips.</small></div></li></ol>
      <aside className="payroll-boundary"><strong>Clear boundary</strong><span>SalarySabi prepares calculations and records. It does not move money or submit remittances.</span></aside>
    </div>
    <div className="payroll-auth-panel"><span className="eyebrow">Private employer workspace</span><h2>{authMode === "signup" ? "Create your payroll account" : "Continue to payroll"}</h2><p>{authMode === "signup" ? "Set up the business, then add your first employee." : "Sign in to return to your team and payroll history."}</p><div className="payroll-auth-toggle"><button className={authMode === "signin" ? "active" : ""} onClick={() => setAuthMode("signin")} type="button">Sign in</button><button className={authMode === "signup" ? "active" : ""} onClick={() => setAuthMode("signup")} type="button">Create account</button></div><form onSubmit={authenticate}><label>Email<input name="email" type="email" autoComplete="email" required /></label><label>Password<input name="password" type="password" autoComplete={authMode === "signup" ? "new-password" : "current-password"} minLength={8} required /></label><button className="primary-button" disabled={busy} type="submit">{authMode === "signup" ? "Create payroll account" : "Sign in"}</button><small>Your organisation and payroll records are available only inside your authenticated workspace.</small></form><p className="payroll-message" role="status">{message}</p></div>
  </section>;
