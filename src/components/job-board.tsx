...
export function JobBoard() {
  ...
  useEffect(() => {
    ...
    fetch(`${url.replace(/\/$/, "")}/rest/v1/jobs?select=${fields}&order=published_at.desc`, {
      ...
      .then((rows) => { setJobs(rows); setState("ready"); })
  ...
  return (
    <>
      <div className="job-search" role="search">
        <label><span>Search jobs</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Job title, skill, company or location" /></label>
      ...
      {state === "loading" && <div className="jobs-state">Checking current verified jobs…</div>}
      {state === "error" && <div className="jobs-state error">Jobs could not be loaded right now. Please try again later.</div>}
      {state === "ready" && visible.length === 0 && <div className="jobs-state"><strong>No matching verified jobs yet.</strong><span>Change the search or ask an employer to submit a salary-transparent role.</span></div>}
      {state === "ready" && visible.length > 0 && <p className="job-result-count">{visible.length} matching {visible.length === 1 ? "job" : "jobs"}</p>}
            ...
            <article className="job-card" key={job.id}>
              ...
              {gross && (
                <div className="take-home-preview">
                  ...
                  <strong>{money.format(estimatedMonthlyAfterPaye(gross.minimum))}–{money.format(estimatedMonthlyAfterPaye(gross.maximum))} / month</strong>
                  <small>Excludes pension, NHF, NHIS and other deductions. Your actual result depends on eligible deductions.</small>
                </div>
              )}
              <p className="job-card-summary">{job.description.slice(0, 220)}{job.description.length > 220 ? "..." : ""}</p>
              <div className="job-card-footer">
                <span>Source checked {new Date(job.source_verified_at).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</span>
                <span>Closes {new Date(job.expires_at).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</span>
                <Link href={`/jobs/${job.slug}`}>Read full details</Link>
                <a href={job.application_url} target="_blank" rel="noopener noreferrer" onClick={() => track("job_apply_clicked")}>
